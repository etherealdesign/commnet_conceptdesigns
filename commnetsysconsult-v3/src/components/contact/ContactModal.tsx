...
export function ContactModal() {
  ...
  return (
    <dialog
      ...
      className="m-0 h-dvh max-h-none w-screen max-w-none bg-transparent p-0 text-ink backdrop:bg-ink/60"
      onClick={(e) => {
    ...
    >
      <div className="flex h-full items-end justify-center md:items-center">
        <div
          ref={panel}
          data-lenis-prevent
          className="relative flex max-h-[92dvh] w-full flex-col overflow-y-auto rounded-t-lg bg-cream md:max-h-[88dvh] md:w-[min(920px,calc(100vw-48px))] md:rounded-lg"
        >
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-ink/10 bg-cream px-5 py-4 md:px-8">
            <h2 className="text-16 md:text-21">Get in contact</h2>
            <button
              type="button"
              className="cursor-pointer text-13 text-grey transition-colors hover:text-ink"
          ...
          <div className="grid gap-10 px-5 py-8 md:grid-cols-[1fr_1.6fr] md:px-8 md:py-10">
            <div className="flex flex-col gap-6">
              <p className="text-16 leading-[1.4] md:text-21">Send us the drawings. An engineer replies.</p>
              <ul className="flex flex-col gap-5 text-13 text-grey">
                {site.offices.map((o) => (
                  <li key={o.id}>
                    <p className="text-ink">
                      {o.city} · {o.id === 'dubai' ? 'HQ' : 'Engineering centre'}
                    </p>
                    <a href={`tel:${o.phone.replace(/\s/g, '')}`} className="u-num block hover:text-ink">
                      {o.phone}
                    </a>
                    <a href={`mailto:${o.email}`} className="block hover:text-ink">
                      {o.email}
                    </a>
                  </li>
                ))}
