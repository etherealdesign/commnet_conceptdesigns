...
function Field({
  ...
  const cls =
    'w-full border border-ink/20 bg-transparent px-3 py-3 text-13 text-ink outline-none transition-colors duration-300 placeholder:text-grey/70 focus:border-primary'
  return (
    <label className="flex flex-col gap-2">
      <span className="mono text-grey">
        {label}
        {required && ' *'}
      </span>
      {as === 'textarea' ? (
  ...
  if (submitted) {
    return (
      <div ref={boxRef} className={cn('flex min-h-80 flex-col items-center justify-center text-center', className)}>
        ...
        <h3 className="d-3 mt-6">Enquiry received.</h3>
  ...
  return (
    <form onSubmit={handleSubmit} className={cn('flex flex-col gap-6', className)}>
      ...
      <fieldset className="flex flex-col gap-2">
        <legend className="mono mb-2 text-grey">Project type</legend>
        <div className="flex flex-wrap gap-2">
          {PROJECT_TYPES.map((t) => (
            <label
              key={t}
              className="chip chip--line cursor-pointer transition-colors duration-300 has-checked:bg-ink has-checked:text-white has-focus-visible:outline-2 has-focus-visible:outline-primary"
            >
      ...
      <label className="flex flex-col gap-2">
        <span className="mono text-grey">Upload BoQ (optional)</span>
        <input
          name="boq"
          type="file"
          className="block w-full text-13 text-grey file:mr-4 file:text-13 file:cursor-pointer file:border file:border-ink/20 file:bg-transparent file:px-4 file:py-2 file:text-ink"
        />
      </label>
      <Field label="Message" name="message" as="textarea" required />
      <div className="flex items-center justify-between gap-6">
        <p className="text-13 text-grey">Scope, a BoQ or a site address is enough to start.</p>
        <Button type="submit" disabled={busy} glyph="↗">
          Send enquiry
        </Button>
      </div>
    </form>
