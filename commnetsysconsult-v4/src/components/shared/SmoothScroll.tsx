...
export function SmoothScroll({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // refresh trigger positions once the new page has laid out, then
    // re-derive the header's colour from what is actually under it
    const id = window.setTimeout(() => {
      ScrollTrigger.refresh()
      syncDark()
    }, 120)
    return () => window.clearTimeout(id)
  }, [pathname, hash, lenis, syncDark])
