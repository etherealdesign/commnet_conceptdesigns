...
function Field({
  ...
  const cls =
    'w-full border border-line bg-lighter px-3 py-3 t-body outline-none transition-colors duration-300 focus:border-fg'
  return (
  ...
  if (submitted) {
    return (
      <div ref={boxRef} className={cn('flex min-h-80 flex-col items-center justify-center text-center', className)}>
        ...
        <h3 className="t-h3 mt-6">Enquiry received.</h3>
        <p className="t-body mt-3 max-w-xs text-fg-muted">An engineer reads every enquiry. We will be in touch shortly.</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className={cn('flex flex-col gap-6', className)}>
      ...
      <fieldset className="flex flex-col gap-2">
        <legend className="mono mb-2 text-fg-muted">Project type</legend>
        <div className="flex flex-wrap gap-2">
          {PROJECT_TYPES.map((t) => (
            <label
              key={t}
              className="chip chip--line cursor-pointer transition-colors duration-300 has-checked:bg-accent has-checked:text-light has-focus-visible:outline-2 has-focus-visible:outline-accent"
            >
      ...
      <label className="flex flex-col gap-2">
        <span className="mono text-fg-muted">Upload BoQ (optional)</span>
        <input
          name="boq"
          type="file"
          className="mono block w-full text-fg-muted file:mr-4 file:cursor-pointer file:border file:border-line file:bg-transparent file:px-4 file:py-2 file:font-mono file:uppercase file:text-fg"
        />
      </label>
      <Field label="Message" name="message" as="textarea" required />
      <div className="flex items-center justify-between gap-6">
        <p className="t-small text-fg-muted">Scope, a BoQ or a site address is enough to start.</p>
