 ...
 * the viewport).
 */
export function Block({ children, className, style, as = 'section', anchor, isDark = false, ariaLabel, ref }: BlockProps) {
  const local = useRef<HTMLElement | null>(null)
  const { pushDark, popDark, setActiveAnchor, headerCenter } = useHeaderStore()
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      onEnter: pushDark,
      onLeave: popDark,
      onEnterBack: pushDark,
      onLeaveBack: popDark,
    })
    return () => {
      if (st.isActive) popDark()
      st.kill()
    }
  }, [isDark, headerCenter, pushDark, popDark])
